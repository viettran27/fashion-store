...
const Navigation = () => {
          ...
          <div className="mt-6">
            {
              routes.map((route: RouteContainer, index: number) => {
                switch (route.type) {
                        ...
                        <CollapsibleContent>
                          {
                            route.options?.map((option: Route, index: number) => {
                              return <Link key={index} href={option.href as Url}>
                                  <div className="font-medium text-md py-1 ml-4 hover:bg-gray-100 px-2 rounded-md">{option.categories}</div>
                              </Link>
                            })
                          }
                        </CollapsibleContent>
                  ...
                  default:
                    return <Link key={index} href={route.href as Url}>
                      <div className="font-medium text-lg py-2 hover:bg-gray-100 rounded-md px-2">{route.categories}</div>
                    </Link>
                }
              })
            }
  ...
  return <div className="space-x-6 flex">
    {
      routes.map((route: RouteContainer, index: number) => {
        switch (route.type) {
          case MenuType.DROPDOWN:
